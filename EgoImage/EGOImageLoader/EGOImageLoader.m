...
@synthesize maxNumberOfImagesAllowedInMemory;
@synthesize dumpRatio;
...
{
    self.dumpRatio = memDumpRatio;
    self.maxNumberOfImagesAllowedInMemory = maxNumOfImgs;
}
...
-(void) addNewObjToImagesDict : (UIImage*) newImage forKey : (NSString*) key
{
    ...
    if([self.imagesDict count]>self.maxNumberOfImagesAllowedInMemory){
        NSLog(@"More than %d images in memory, count is %d! Dumping!!!",self.maxNumberOfImagesAllowedInMemory,[self.imagesDict count]);
