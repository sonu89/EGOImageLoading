...
- (void)start {
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:@"egoimageloaderstarteddownloadinganimage" object:self userInfo:[NSDictionary dictionaryWithObjectsAndKeys:
                              self.imageURL,@"urlbeingused",
                                                                                                                               nil]];
	[request release];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    [[NSNotificationCenter defaultCenter]postNotificationName:@"egoimageloaderfinisheddownloadinganimage" object:self userInfo:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                                                                               self.imageURL,@"urlbeingused",
                                                                                                                               nil]];
	if(connection != _connection) return;
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
	[[NSNotificationCenter defaultCenter]postNotificationName:@"egoimageloaderfinisheddownloadinganimage" object:self userInfo:[NSDictionary dictionaryWithObjectsAndKeys:
                                                                                                                                ...
                                                                                                                                nil]];
    if(connection != _connection) return;
	NSLog(@"%@ - %@", NSStringFromSelector(_cmd), [error userInfo]);
	if([self.delegate respondsToSelector:_cmd]) {
